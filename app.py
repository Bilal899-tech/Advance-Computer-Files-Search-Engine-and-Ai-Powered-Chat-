...
import threading
import webbrowser
from core import Config, VectorStore, Database, DocumentProcessor, detect_hardware, LLMClient
...
class ChatApp:
    ...
    def create_status_bar(self, parent):
        ...
        # Ollama status
        self.ollama_status_label = tk.Label(status_items, text="🔴 SPUR API", 
        ...
        self.model_label = tk.Label(status_items, 
                                    ...
                                    bg=COLORS['surface_container_low'])
        self.provider_label = tk.Label(status_items,
                                       text="Provider: --",
                                       font=("Inter", 10), fg=COLORS['on_surface_variant'],
                                       bg=COLORS['surface_container_low'])
        self.provider_label.pack(side=tk.LEFT, padx=(0, 4))
        
        self.retry_spur_btn = tk.Button(status_items, text="↻ SPUR",
                                        command=self._retry_spur,
                                        bg=COLORS['surface_container'], fg=COLORS['primary'],
                                        font=("Inter", 9, "bold"),
                                        relief=tk.FLAT, cursor='hand2',
                                        padx=6, pady=2)
        self.retry_spur_btn.pack(side=tk.LEFT, padx=(0, 8))
    ...
    def check_system_status(self):
        # Probe provider once — auto-detects SPUR or local Ollama
        self.llm_client.probe_spur()
        self.ollama_connected = True
        provider = self.llm_client.active_provider
        if provider == "spur":
            self.ollama_status_label.config(text="🟢 SPUR API", fg=COLORS['success'])
        else:
            self.ollama_status_label.config(text="🟢 Ollama (local)", fg=COLORS['primary'])
    ...
    def _retry_spur(self):
        """Re-probe SPUR. If it just became available, ask user to switch."""
        just_reachable = self.llm_client.recheck_spur()
        self.check_system_status()
        if just_reachable:
            switch = messagebox.askyesno(
                "SPUR Available",
                "SPUR API is now reachable. Would you like to switch to SPUR as the primary provider?"
            )
            if switch:
                self.llm_client.probe_spur()
                self.check_system_status()
                messagebox.showinfo("Provider Switched", "Now using SPUR API for inference.")
        else:
            if self.llm_client.active_provider == "spur":
                messagebox.showinfo("Provider", "Already connected to SPUR API.")
            else:
                messagebox.showinfo("Provider", "SPUR API is still unreachable. Using local Ollama.")

    def _update_model_display(self):
        """Update the model label in the status bar."""
        tier = self.config.search.get('model_tier', 'low')
        provider = getattr(self.llm_client, 'active_provider', '?')
        model_name = self.config.models.get('search_chat', self.config.models['chat'])
        if provider == 'local':
            model_name = self.config.models.get('chat_fallback', model_name)
        self.model_label.config(text=f"Model: {model_name} ({tier})")
        label = "SPUR" if provider == "spur" else "Ollama (local)"
        color = COLORS['success'] if provider == "spur" else COLORS['primary']
        self.provider_label.config(text=f"Provider: {label}", fg=color)
    ...
    def _process_message(self, user_text):
        try:
            ...
            try:
                ...
                fallback_model = self.config.models.get('chat_fallback', 'qwen2.5:3b')
                ...
                ai_response = f"Sorry, no model providers are available. Error: {str(e)}"
                confidence = 0
                latency = 0
                search_results = []
